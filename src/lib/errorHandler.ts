...
export class ErrorHandler {
  ...
  static parseError(error: unknown): AppError {
    ...
    if (errorMessage.includes("Freighter") || errorMessage.includes("wallet")) {
      if (errorMessage.includes("not installed")) {
        return {
          ...
          retryable: false,
          actionable:
            "Install Freighter wallet extension from the Chrome Web Store.",
        };
      }
      ...
      if (errorMessage.includes("network")) {
        return {
          ...
          retryable: false,
          actionable:
            "Please switch to Stellar Testnet in your Freighter wallet settings.",
        };
      }
    ...
    if (
      errorMessage.includes("minimum lock period") ||
      errorMessage.includes("lock period not met")
    ) {
      return {
        ...
        retryable: false,
        actionable:
          "Please wait for the minimum lock period to expire before unlocking.",
      };
    }

    if (
      errorMessage.includes("already locked") ||
      errorMessage.includes("position exists")
    ) {
      return {
        ...
        retryable: false,
        actionable:
          "Check your existing position or unlock before locking new assets.",
      };
    }

    if (
      errorMessage.includes("not unlockable") ||
...
export async function retryWithBackoff<T>(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      console.warn(
        `Retrying ${context} in ${delay}ms (attempt ${attempt}/${maxAttempts})`,
      );
  ...
  throw lastError ?? ErrorHandler.parseError("Retry exhausted without error");
}
